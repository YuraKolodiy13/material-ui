import React, {useState, useContext, useEffect, Fragment} from 'react'
import Button from '@material-ui/core/Button';
import {MyContext} from '../../index'
import UnMount from "./unMount";
import './Hooks.scss'


const colorArray = ['#FF6633', '#FFB399', '#FF33FF', '#FFFF99', '#00B3E6',
  '#E6B333', '#3366E6', '#999966', '#99FF99', '#B34D4D',
  '#80B300', '#809900', '#E6B3B3', '#6680B3', '#66991A',
  '#FF99E6', '#CCFF1A', '#FF1A66', '#E6331A', '#33FFCC',
  '#66994D', '#B366CC', '#4D8000', '#B33300', '#CC80CC',
  '#66664D', '#991AFF', '#E666FF', '#4DB3FF', '#1AB399',
  '#E666B3', '#33991A', '#CC9999', '#B3B31A', '#00E680',
  '#4D8066', '#809980', '#E6FF80', '#1AFF33', '#999933',
  '#FF3380', '#CCCC00', '#66E64D', '#4D80CC', '#9900B3',
  '#E64D66', '#4DB380', '#FF4D4D', '#99E6E6', '#6666FF'];


const Hooks = () => {

  const [color, setColor] = useState('white');
  const [fontSize, setFontSize] = useState('16px');
  const [visible, setVisible] = useState(true);
  const [someValue, setSomeValue] = useState(1);


  const value = useContext(MyContext);

  const handleColor = () =>{
    setColor(colorArray[Math.floor(Math.random() * colorArray.length)])
  };

  useEffect(() => {
    console.log('mount');
    document.querySelector('.notification').classList.remove('hide');
    setTimeout(() => document.querySelector('.notification').classList.add('hide'), 5000)
    return () => {
      console.log('componentWillUnmount');
    }
  }, []);

  useEffect(() => {
    console.log('update')
  }, [someValue]);

  return(
    <div style={{ backgroundColor: color}} >
      <Button onClick={handleColor} variant="contained" color="primary">change color</Button>
      <Button onClick={() => setFontSize((s) => parseInt(s) + 1 + 'px')} variant="contained" color="primary" style={{fontSize: fontSize}}>increase font size</Button>
      <Button variant="contained" color="primary">{value}</Button>

      {visible
      ? <Fragment>
          <div style={{display: 'flex', justifyContent: 'center', marginTop: '50px'}}>
            <Button variant="contained" color="secondary" onClick={() => setSomeValue((v) => v + 1)}>+</Button>
            <Button variant="contained" color="secondary" onClick={() => setVisible(false)}>hide</Button>
          </div>
          <UnMount someValue={someValue}/>
        </Fragment>
      : <Fragment>
          <div style={{display: 'flex', justifyContent: 'center', marginTop: '50px'}}>
            <Button variant="contained" color="secondary" onClick={() => setVisible(true)}>show</Button>
          </div>
        </Fragment>}

      <div className="notWrap">
        <div className='notification hide'>
          <p>notification</p>
        </div>
      </div>

    </div>
  )
};

export default Hooks